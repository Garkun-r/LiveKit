...
: prompt lookup/cache через Directus упал или не дал usable prompt, агент
  использовал file prompt.
...
: после старта `AgentSession` не появилось ни речи робота, ни речи клиента,
  ни transcript-событий до startup watchdog timeout. Агент закрывает комнату
  с `close_reason=no_dialog_startup_timeout`.

`speech_playout_timeout`
: явный speech-шаг робота был создан, но `SpeechHandle.wait_for_playout()` не
  завершился до `VOICE_SPEECH_PLAYOUT_TIMEOUT_SEC`. Агент останавливает handle,
  пишет incident и не оставляет entrypoint ждать бесконечно.

`speech_playout_failed`
: явный speech-шаг или prerecorded audio не удалось поставить или дождаться
  из-за ошибки вне timeout. Используется для voice pipeline failures, где
  realtime path должен продолжить cleanup без зависания.

`initial_greeting_failed`
: первое приветствие не завершилось ни через pre-synthesized audio, ни через
  fallback `generate_reply`; агент закрывает звонок с понятной причиной.
