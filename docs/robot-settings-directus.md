...
- API-ключи и credential refs провайдеров: `GOOGLE_API_KEY`, `XAI_API_KEY`,
  ...
  `SBER_SALUTESPEECH_AUTH_KEY`, `TBANK_VOICEKIT_*`, legacy `VOICEKIT_*`,
...
Для Asterisk в текущем snapshot заведены отдельные профили под локальный
контур:
...
Proxy-профили содержат `egress=proxy`, но не все Asterisk-провайдеры идут через
proxy. В локальном профиле snapshot сейчас `runtime=asterisk` выбирает
`llm.primary=llm_xai_proxy`, `llm_routing.fast=llm_xai_proxy`,
`llm_routing.complex=llm_xai_proxy`, `stt.primary=stt_yandex_ru` и
`tts.primary=tts_tbank_ru`. Yandex/T-Bank/Sber по умолчанию идут direct через
`src/egress.py`.
