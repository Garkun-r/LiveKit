...
class EarlyInterimFinalStream(stt.SpeechStream):
    ...
    async def _notify_local_end_of_speech(
        ...
        async with self._state_lock:
            ...
            await self._cancel_local_eos_task_locked()
            self._local_eos_due = False
            remaining_delay_sec = self._remaining_delay_from_ended_at(ended_at)
            self._local_eos_deadline = asyncio.get_running_loop().time() + remaining_delay_sec
            self._local_eos_task = asyncio.create_task(
                self._delayed_emit_local_synthetic_final(
                    ended_at=ended_at,
                    delay_sec=remaining_delay_sec,
                ),
    ...
    async def _delayed_emit_local_synthetic_final(
        ...
        *,
        ...
        try:
            await asyncio.sleep(self._delay_sec if delay_sec is None else delay_sec)
            async with self._state_lock:
    ...
    def _remaining_delay_from_ended_at(self, ended_at: float | None) -> float:
        if ended_at is None:
            return self._delay_sec
        return max(0.0, ended_at + self._delay_sec - time.time())
    ...
    def _log_late_interim_locked(self, ev: stt.SpeechEvent) -> None:
        text = _event_text(ev)
        logger.info(
            "late interim transcript suppressed after final",
            extra={
                "request_id": ev.request_id,
                "characters_count": len(text),
                "synthetic_committed": self._synthetic_committed,
            },
        )
